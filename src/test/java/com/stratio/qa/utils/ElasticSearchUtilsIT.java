...
//        assertThat(es_utils.getClient().admin().cluster().prepareNodesInfo().all().execute()).isNotNull();
    ...
    @Test
    public void createMappingTest() throws IOException, InterruptedException {
        es_utils.connect();
        if (es_utils.indexExists("testindex")) {
            es_utils.dropSingleIndex("testindex");
        }
    ...
    @Test
    public void searchFilterEquals() throws IOException, InterruptedException {
        es_utils.connect();
        if (es_utils.indexExists("testindex")) {
            es_utils.dropSingleIndex("testindex");
        }
        ...
        int res = 0;
        try {
            res = es_utils.searchSimpleFilterElasticsearchQuery("testindex", "ident", "1", "equals").size();
        } catch (Exception e) {
            e.printStackTrace();
        }
